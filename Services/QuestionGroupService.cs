...
namespace TINWeb.Services
{
    public class QuestionGroupService
    {
            ...
            bool clearImage3)
        {
            ...
            existing.EnforceUniqueSelection = record.EnforceUniqueSelection;
...
BEGIN
    ALTER TABLE [dbo].[QuestionGroup] ADD [EnforceUniqueSelection] [bit] NOT NULL CONSTRAINT [DF_QuestionGroup_EnforceUniqueSelection] DEFAULT (0);
END;
...
IF COL_LENGTH('dbo.QuestionGroup', 'EnforceUniqueSelection') IS NOT NULL
BEGIN
    UPDATE [dbo].[QuestionGroup]
    ...
    IF EXISTS (
        ...
        FROM sys.columns
        WHERE object_id = OBJECT_ID(N'[dbo].[QuestionGroup]')
          AND name = 'EnforceUniqueSelection'
          AND is_nullable = 1)
    BEGIN
        ALTER TABLE [dbo].[QuestionGroup] ALTER COLUMN [EnforceUniqueSelection] [bit] NOT NULL;
    END;
    ...
    BEGIN
        ALTER TABLE [dbo].[QuestionGroup]
        ADD CONSTRAINT [DF_QuestionGroup_EnforceUniqueSelection] DEFAULT (0) FOR [EnforceUniqueSelection];
    END;
END;
